...
H.bridge = (() => {
  ...
  setInterval(() => {
    let changed = false;
    for (const [o, b] of bridges) { for (const [src, info] of b.sources) if (Date.now() - info.ts > 12000 || isClosed(src)) { b.sources.delete(src); changed = true; } if (!b.sources.size) { bridges.delete(o); changed = true; } }
  ...
  function fetch(url, init = {}) {
    ...
    const b = bridges.get(origin); const x = b && best(b);
    if (!x) return Promise.reject(new Error(`No browser bridge connected for ${origin}. Open a ${origin} tab where you are logged in and click the "LLM Harness bridge" bookmarklet (plugin setup, step 3).`));
    const id = H.uid();
    note('request ' + id + ' ' + (init.method || 'GET') + ' ' + url.replace(origin, ''));
    const started = Date.now();
    H.bus.emit('bridge-request', { id, origin, url, phase: 'sent' });
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => { pending.delete(id); note('timeout ' + id); H.bus.emit('bridge-request', { id, origin, url, phase: 'timeout' }); reject(new Error(`Bridge request timed out after 20 s: the ${origin} tab received no answer or did not respond. Check the blue bar on that tab (it shows request counts and the last error); make sure the tab is open and not on a login page.`)); }, init.timeout || 20000);
...
if(!alive(st.w)){
 ...
 clearInterval(st.t);const hello=()=>{try{st.w.postMessage({type:'llm-bridge-hello',origin:O,mode:st.mode},st.ack?st.T:'*')}catch(e){}};st.t=setInterval(()=>{hello();paint();},3000);setTimeout(hello,300);st.t0=Date.now();
...
if(!b.querySelector('[data-x]')){const x=document.createElement('span');x.dataset.x=1;x.textContent='  \\u2715';x.style.cssText='cursor:pointer;margin-left:12px';x.onclick=()=>{clearInterval(st.t);if(st.fr)st.fr.remove();delete window.__llmBridge;b.remove();};b.append(x);}})();`;
  ...
  return { fetch, ping, has, list, bookmarklet, openSite, diagnostics, embedded: () => window.top !== window || location.hash.includes('embedded') };
})();
