...
H.code = (() => {
  ...
  function matchesIgnore(gitignoreText, path, isDir = false) {
    const saved = ignoreRules;
    try { ignoreRules = compile(gitignoreText, ''); return ignoredBy(path, isDir); } finally { ignoreRules = saved; }
  }
  function ignoredBy(path, isDir) {
    let hit = false;
  ...
  async function index({ refresh = false } = {}) {
    if (!H.fs.hasRoot() && !H.fs.virtual.size) throw new Error('No workspace folder is open. Ask the user to click "Workspace" in the top bar and pick the project folder; code and file tools work only inside that folder.');
  ...
  const snapKey = () => 'snapshot:' + (H.fs.name() || 'virtual');
  ...
  async function snapshot({ note = '' } = {}) {
    ...
    const rec = { taken: Date.now(), note, workspace: H.fs.name() || '(virtual)', files: store, fileCount: Object.keys(store).length, skipped, truncated };
  ...
  async function refreshContext() {
    const name = H.fs.name() || '';
    const movedFolder = name !== contextFor;       // a different project with an AGENTS.md is still worth announcing
    contextFor = name;
    const prev = contextCache;
    contextCache = await contextFile();
  ...
  function promptSection() {
    if (!H.fs.hasRoot() && !H.fs.virtual.size) return '';
...
- Delegate wide sweeps over many files to run_subagent so the main conversation keeps its context.`;
  ...
  H.bus?.on?.('workspace', () => {
    bump(); idx = null;
    H.git?.resetCaches?.();
    H.git?.quickBranch?.().catch(() => { });     // one small file read: the branch for the top bar and the prompt
    refreshContext().catch(() => { });
  });

  return {
    generation: () => generation, bump, index, read, search, find, outline, outlineGlob, outlineText, symbol, deps,
    overview, contextFile, refreshContext, promptSection, snapshot, getSnapshot, changes,
    langOf, ignoredBy, matchesIgnore, searchable, DEFAULT_SKIP, BINARY_EXT, RULES,
  };
})();
